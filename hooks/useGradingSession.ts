...
export const useGradingSession = (step: 'setup' | 'task' | 'grading' | 'report') => {
  ...
  const [parts, setParts] = useState<Part[]>(['1']);
  const [taskDescriptions, setTaskDescriptions] = useState<Record<Part, string>>({ '1': '' });
  const [taskImages, setTaskImages] = useState<Record<Part, string>>({});
  ...
  const classStats = useMemo(() => {
    ...
    // Per-part stats
    const partStats: Record<string, { avg: string; count: number; catAvgs: Record<string, string> }> = {};
    parts.forEach(p => {
      const pStudents = students.filter(s => s.part === p);
      if (pStudents.length === 0) return;
      const pTotals = pStudents.map(s => s.scores.content + s.scores.communicative + s.scores.organisation + s.scores.language);
      partStats[p] = {
        avg: (pTotals.reduce((a, b) => a + b, 0) / pStudents.length).toFixed(1),
        count: pStudents.length,
        catAvgs: {
          content: (pStudents.reduce((a, b) => a + b.scores.content, 0) / pStudents.length).toFixed(1),
          communicative: (pStudents.reduce((a, b) => a + b.scores.communicative, 0) / pStudents.length).toFixed(1),
          organisation: (pStudents.reduce((a, b) => a + b.scores.organisation, 0) / pStudents.length).toFixed(1),
          language: (pStudents.reduce((a, b) => a + b.scores.language, 0) / pStudents.length).toFixed(1),
        },
      };
    });
  ...
  }, [sessionId, createdAt, sessionName, graderName, level, parts, taskDescriptions, taskImages, students, currentStudentIndex, currentImageIndex, currentStudentTime]);
  ...
  const resumeSession = (session: SessionRecord) => {
    ...
    setTaskDescriptions(session.taskDescriptions || (session.parts.length > 0 ? { [session.parts[0]]: '' } : { '1': '' }));
    setTaskImages(session.taskImages || {});
  ...
  const saveStudentRealName = (realName: string) => {
    setStudents(prev => prev.map((s, idx) => idx === currentStudentIndex ? { ...s, realName, name: `${realName} (Part ${s.part})` } : s));
  };

  const saveStudentPart = (part: Part) => {
    setStudents(prev => prev.map((s, idx) => idx === currentStudentIndex ? { ...s, part, isCover: false, name: `${s.realName || 'Untitled'} (Part ${part})` } : s));
  };

  const saveStudentIsCover = () => {
    setStudents(prev => prev.map((s, idx) => idx === currentStudentIndex ? { ...s, isCover: true, name: `${s.realName || 'Untitled'} (Non-answer)` } : s));
  };
  ...
  const mergeNextStudent = () => {
    ...
    if (currentStudent && next.part !== currentStudent.part) return;
    setStudents(prev => {
      const updated = [...prev];
      const current = { ...updated[currentStudentIndex] };
      const nextStudent = { ...updated[currentStudentIndex + 1] };
      const currentImageCount = current.images.length;
      const adjustedAnnotations = nextStudent.annotations.map(ann => ({ ...ann, pageIndex: ann.pageIndex + currentImageCount }));
      updated[currentStudentIndex] = { ...current, images: [...current.images, ...nextStudent.images], annotations: [...current.annotations, ...adjustedAnnotations] };
  ...
  const mergeWithPrevious = () => {
    ...
    if (currentStudent && prev.part !== currentStudent.part) return;
    setStudents(prev => {
      const updated = [...prev];
  ...
  const splitStudentAtPage = () => {
    ...
    setStudents(prev => {
      ...
      const newStudent: Student = { ...current, id: (Date.now() + Math.random()).toString(), name: `${current.name} (Part 2)`, realName: current.realName, images: movedImages, annotations: movedAnnotations, scores: { content: 0, communicative: 0, organisation: 0, language: 0 }, justifications: { content: '', communicative: '', organisation: '', language: '' }, timeSpent: 0 };
  ...
  const handleDownloadGradebook = useCallback((filteredStudents: Student[]) => {
    const headers = ['Name', 'Part', 'Content', 'Communicative', 'Organisation', 'Language', 'Total', 'Time (s)'];
    const rows = filteredStudents.map(s => {
      const total = s.scores.content + s.scores.communicative + s.scores.organisation + s.scores.language;
      return [s.realName || s.name, `Part ${s.part}`, s.scores.content, s.scores.communicative, s.scores.organisation, s.scores.language, total, s.timeSpent];
    });
  ...
  const handlePrintBatchFeedback = useCallback(async (filteredStudents: Student[]) => {
    ...
    filteredStudents.forEach((s, idx) => {
      ...
      doc.setFontSize(18);
      doc.text(`Feedback: ${s.realName || s.name}`, 20, 20);
      doc.setFontSize(12);
      doc.text(`Level: ${level} | Part: ${s.part}`, 20, 30);
  ...
  const handleTaskUpload = async (part: Part, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (base64Image) {
        setTaskImages(prev => ({ ...prev, [part]: base64Image }));
        const text = await transcribeTaskImage(base64Image);
        setTaskDescriptions(prev => ({ ...prev, [part]: text }));
      }
    } catch (error) { alert("Failed."); } finally { setIsTaskLoading(false); }
  };

  const handlePasteTaskImage = useCallback(async (part: Part, file: File) => {
    setIsTaskLoading(true);
    try {
      const base64Image = await new Promise<string>((resolve) => { const reader = new FileReader(); reader.onload = (ev) => resolve(ev.target?.result as string); reader.readAsDataURL(file); });
      setTaskImages(prev => ({ ...prev, [part]: base64Image }));
      const text = await transcribeTaskImage(base64Image);
      setTaskDescriptions(prev => ({ ...prev, [part]: text }));
    } catch (error) { alert("Failed to process pasted image."); } finally { setIsTaskLoading(false); }
  }, []);
  ...
  const handleScriptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (images.length > 0) {
          images.forEach((img) => {
            const idx = startIdx + totalCount;
            newStudents.push({
              id: `${Date.now()}-${i}-${Math.random()}-${totalCount}`,
              name: `Untitled ${idx} (Part ${defaultPart})`,
              realName: `Untitled ${idx}`,
              part: defaultPart,
              isCover: false,
              images: [img],
              annotations: [],
              scores: { content: 0, communicative: 0, organisation: 0, language: 0 },
              justifications: { content: '', communicative: '', organisation: '', language: '' },
              timeSpent: 0
            });
            totalCount++;
          });
  ...
  return {
    ...
    level, setLevel,
    parts, setParts,
    taskDescriptions, setTaskDescriptions,
